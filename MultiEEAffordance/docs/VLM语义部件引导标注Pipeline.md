...
> 2026-05-19 更新：远程实验显示 Florence-2 + SAM2 虽然可以跑通，但在稀疏点云渲染图上仍容易把目标部件定位成整物体或大面积主体区域。因此当前推荐路线已升级为 `VLM 候选选择 Pipeline v2`：先由 3D 几何和已有弱标签生成通用候选区域，再让 VLM 选择候选编号，并通过执行器机制规则过滤。详见 `docs/VLM候选选择Pipeline_v2设计与汇报.md`。本文档保留作为 v1 语义部件路线的复盘与历史依据。

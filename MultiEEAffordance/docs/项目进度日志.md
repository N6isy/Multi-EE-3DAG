...
- 修改 `run_qwen3vl_sam2_pilot.py`：
  ...
  - 增加 foreground prompt refine：点和框若落到黑色背景，会被贴近真实点云前景；距离过远则丢弃。
...
- 本地提交并推送。
- 远程服务器 `git pull` 后，先执行 `render_vlm_pilot_views.py --pilot-id vlm_pilot_005 --image-size 768 --point-size 2 --overwrite`。

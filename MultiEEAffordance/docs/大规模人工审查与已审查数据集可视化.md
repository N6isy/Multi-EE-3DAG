...
- 当前 v3 标注主线必需：`/home/lzq/Multi-EE-3DAG/MultiEEAffordance/raw/3d_affordancenet/full-shape.zip`
- 后续补充或扩展使用：`/home/lzq/Multi-EE-3DAG/MultiEEAffordance/raw/partnet-mobility/partnet-mobility-v0.zip`

`full-shape.zip` 是当前 3D AffordanceNet full-shape 样本池的核心原始数据。它包含 `full_shape_train_data.pkl` 和 `full_shape_val_data.pkl`，转换后可以生成完整物体点云、原始 affordance 弱标签、样本 manifest 和后续审查队列。当前大规模人工审查默认从这个 zip 生成 `pick_up / open_pull / press_push` 三类任务样本。

PartNet-Mobility 暂时不是当前 v0.1 主线的必需输入。它更适合后续补强 articulated object、handle、button、joint 等结构化部件，不影响当前先用 3D AffordanceNet full-shape 开始人工审查。

注意：审查者如果只负责网页人工标注，不需要自己下载完整 `full-shape.zip`，也不需要重跑 VLM pipeline。维护者应先在服务器端生成候选，再给审查者分发包含点云、候选和样本 JSONL 的最小数据包。
...
```text
processed/annotation_batches/v0_1/merged_refined_samples.jsonl
```

如果同一个 `sample_id + executor` 被两位审查者都保存过，合并前应列入 conflict review，不要自动覆盖。

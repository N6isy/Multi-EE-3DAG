...
从 2026-06-01 起，五任务下采样、网页人工审查和 refined 输出统一在数据存储服务器执行：

```text
10.24.1.11:/home/lzq/data/MultiEEAffordance
```
...
PartNet-Mobility 的原始 object 数量和后续任务展开样本行数必须分开统计。PartNet-Mobility 转换命令见 `PartNet-Mobility数据解压与格式转换.md`。

...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Comma-separated reviewer ids to archive/package. If omitted, package all reviewers. "
            "Use reviewer_b when reviewer_a can access the server data directly."
        ),
    )
    parser.add_argument(
        "--calibration-objects",
...
def package_reviewer(
    ...
    if package_files and not dry_run:
    ...
    return {
        ...
        "missing_references": missing,
        "archive": None if (not package_files or archive_format == "none") else archive_path.as_posix(),
        "package_dir": stage_dir_path.as_posix() if (package_files and archive_format == "none") else None,
    }
...
def main() -> None:
    ...
    if args.package_reviewers:
        package_reviewer_ids = {x.strip() for x in args.package_reviewers.split(",") if x.strip()}
    else:
        package_reviewer_ids = set(reviewer_ids)
    unknown_package_reviewers = sorted(package_reviewer_ids - set(reviewer_ids))
    if unknown_package_reviewers:
        raise ValueError(
            "--package-reviewers contains ids not listed in --reviewers: "
            + ",".join(unknown_package_reviewers)
        )
    ...
    for reviewer_id in reviewer_ids:
        reviewer_summaries[reviewer_id] = package_reviewer(
            ...
            package_files=reviewer_id in package_reviewer_ids,
        )
    ...
    summary = {
        ...
        "package_reviewers": sorted(package_reviewer_ids),

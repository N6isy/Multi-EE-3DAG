...
def relation_loss(
    ...
    for left in range(executor_count):
        for right in range(left + 1, executor_count):
            ...
            weights.append(channel_supervision[:, left] * channel_supervision[:, right])
...
def compute_loss(
    ...
    *,
    ...
    mask_focal = focal_bce(outputs["mask_logits"], batch["masks"], supervision)
    mask_dice = dice_loss(outputs["mask_logits"], batch["masks"], supervision)
    feasibility = feasibility_loss(outputs["feasibility_logits"], batch["feasibility"], supervision)
    relation = relation_loss(outputs["mask_logits"], batch["masks"], supervision)
    total = mask_focal + lambda_dice * mask_dice + lambda_feasibility * feasibility + lambda_relation * relation
    return {
        "total": total,
